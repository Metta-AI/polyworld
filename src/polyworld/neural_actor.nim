## Polyworld neural model format (model.bin, magic GOTANET1) and its
## restricted FP32 MinGRU actor, shared by every game with neural seats.
##
## The layout and equations are PufferLib's MinGRU policy: a linear encoder,
## one MinGRU layer with a highway to the encoder output, and a linear
## decoder whose outputs are the concatenated categorical heads. Gods of the
## Arena introduced the format, so its magic stays GOTANET1 and every model
## trained for it keeps loading. The dimensions (inputs, width, heads) come
## from the model header; a game's contract (neural_package.nim) decides
## which dimensions it accepts. A hosted seat and a native training library
## run this same code, so a trainer can check its own actor against it bit
## for bit. Byte layout: docs/neural-policies.md.
import std/math

type
  Actor* = ref object
    ## A loaded model. Read-only outside this module (getters below), so a
    ## shared actor cannot be resized into out-of-bounds reads.
    inputs, hidden, outputs: int
    heads: seq[int]
    obsContract, actContract: string
...
const
  ...
  DefaultNeuralOpBudget* = 4_000_000
    ## Operations one seat may spend per tick on inference, separate from the
    ## BASIC instruction budget, unless the game's contract says otherwise.
...
proc loadActor*(data: string, opBudget = DefaultNeuralOpBudget): Actor =
  ## Validates and loads a GOTANET1 model; raises ValueError on any defect,
  ## including an inference cost over `opBudget` operations.
  ...
  result.obsContract = data[p..<p+64]; p += 64
  result.actContract = data[p..<p+64]; p += 64
  for hash in [result.obsContract, result.actContract]:
    for c in hash:
      if c notin {'0'..'9', 'a'..'f'}:
  ...
  if ops > opBudget:
    raise newException(ValueError, "neural actor needs " & $ops &
      " operations per inference, over the " & $opBudget & " budget")

proc inputSize*(actor: Actor): int {.inline.} =
  ## Observation floats per inference.
  actor.inputs

proc hiddenSize*(actor: Actor): int {.inline.} =
  ## MinGRU width (recurrent state floats).
  actor.hidden

proc outputSize*(actor: Actor): int {.inline.} =
  ## Logits per inference (sum of the head sizes).
  actor.outputs

proc headSizes*(actor: Actor): seq[int] =
  ## Categorical head sizes, in logit order (a copy).
  actor.heads

proc headCount*(actor: Actor): int {.inline.} =
  actor.heads.len

proc observationContract*(actor: Actor): string =
  ## The observation contract hash stored in the model header.
  actor.obsContract

proc actionContract*(actor: Actor): string =
  ## The action contract hash stored in the model header.
  actor.actContract
...
proc infer*(actor: Actor, obs: openArray[float32], state: var openArray[float32],
  ...
  if obs.len != actor.inputs or state.len != actor.hidden or
      logits.len != actor.outputs:
