## Raw DEFLATE decoding with a hard output limit, for untrusted archives
## (neural packages). Adapted from zippy's zippy/inflate.nim (MIT License,
## Copyright (c) 2020 Ryan Oldenburg); the only change is that every write
## past `limit` bytes raises ZippyError instead of growing the output, so a
## small compressed entry cannot expand into gigabytes before its declared
## size is checked.
...
proc inflateBounded*(src: string, limit: int): string =
  ...
  inflateInto(result, cast[ptr UncheckedArray[uint8]](src[0].unsafeAddr),
    src.len, 0, limit)
