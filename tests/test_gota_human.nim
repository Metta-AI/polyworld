...
proc arena(class = Ranger): Game =
  ...
  for building in result.world.buildings.mitems: building.hp = 0
  ...
  for slot in HeroAbilitySlot:
    ...
    hero.charges[slot] = heroAbility(class, slot).abilitySpec.charges
  hero.state = Marching
...
echo "Testing A and S are not mouse buttons unless mousekeys is selected"
block:
  setControlPreset(MouseControls)
  doAssert playKey(Stop) == KeyS and playKey(AttackMove) == KeyA
  doAssert playKey(SpellQ) == KeyQ
  setControlPreset(MouseKeyControls)
  doAssert playKey(Stop) == KeyX and playKey(AttackMove) == KeyZ
  setControlPreset(MouseControls)

echo "Testing level shortcuts and portal item aiming survive shared input"
block:
  let game = arena()
  let hero = game.world.heroes[0]
  for slot in HeroAbilitySlot:
    hero.abilityLevels[slot] = 0
  var input: HumanInputState
  input.handlePlayerKeys(
    HumanKeys(pressed: {KeyW}, held: {KeyLeftShift}, focused: true),
    game.world, hero.id, hero.id, hero.aimAt, true
  )
  flushPlayerCommands(game)
  ...
  hero.itemCounts[0] = 1
  input.handlePlayerKeys(HumanKeys(pressed: {KeyF}, focused: true),
    game.world, hero.id, hero.id, hero.aimAt, true)
  doAssert armedItem == 0
  resetHumanMatch()
  doAssert armedItem == -1
