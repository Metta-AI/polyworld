## Generated terrain with models and rendering settings from quadterrain_shadows.
## Tiles and stamps use paired 256x256 color and height maps.
## F1 toggles the panel; F12 saves a screenshot under tmp.
## Run with nim r -d:release experiments/terrain/quadterrain_aigen.nim.
...
proc terrainFrag(
  ...
  if roadEnabled > 0.5 and vertSplatRange.z >= 0.0 and worldPos.z >= 8.0:
    let
      distance = roadDistance(worldPos.x, worldPos.z)
      width = roadHalfWidth(worldPos.z)
    if distance < width + 2.5:
      let
        corridor = (1.0 - smoothstep(width + 1.0, width + 2.5, distance)) *
          smoothstep(8.0, 9.5, worldPos.z)
        meadowA = terrainSample(uv, GrassSurface.float32)
        meadowB = terrainSample(uv, 2.0)
        meadowMix = 0.5 + 0.5 * sin(worldPos.x * 0.21 + worldPos.z * 0.17)
        meadow = meadowA * (1.0 - meadowMix) + meadowB * meadowMix
        road = terrainSample(uv, DirtSurface.float32)
        paint = 1.0 - smoothstep(width - 0.35, width + 0.35, distance)
        amount = surfaceAmount(paint, meadow.w, road.w)
        painted = meadow * (1.0 - amount) + road * amount
      surface = surface * (1.0 - corridor) + painted * corridor
  let
...
let roadEnabledLocation = glGetUniformLocation(terrainProgram, "roadEnabled")
...
const TileStyleTable = [
  ...
  TileStyle(
    topMaterial: GravelSurface,
    skirtMaterial: GravelSurface,
    skirtTint: vec3(0.9)
  ),
  TileStyle(
...
proc generateLayers() =
  ...
  proc corner(cx, cz: int): float32 =
    ...
    if fortEnabled and cz >= c + 6:
      # Flatten the same curved corridor that the material shader paints.
      let
        d = roadDistance(cx.float32 - HalfGrid, cz.float32 - HalfGrid)
  ...
  if fortEnabled:
    ...
    for z in 0 ..< GridTiles:
      for x in 0 ..< GridTiles:
        ...
        if r <= 8:
          # The courtyard and gate use the dirt road material.
          gtile(x, z).tops = pack([plateau, plateau, plateau, plateau])
          gtile(x, z).kind = RoadTile
        let
          roadZ = z.float32 + 0.5'f - HalfGrid
          distance = roadDistance(x.float32 + 0.5'f - HalfGrid, roadZ)
        if z >= c + 9 and distance < roadHalfWidth(roadZ) + 2.5'f:
          gtile(x, z).kind = GrassTile
        if (gateColumn and z in c + 6 .. c + 8) or
          (z >= c + 9 and distance < roadHalfWidth(roadZ) + 0.6'f):
            gtile(x, z).kind = RoadTile
    ...
    for z in 0 ..< GridTiles:
      for x in 0 ..< GridTiles:
        ...
        if fortEnabled and max(abs(x - c), abs(z - c)) <= 9:
          continue
        if fortEnabled and z >= c + 9 and roadDistance(
          x.float32 + 0.5'f - HalfGrid,
          z.float32 + 0.5'f - HalfGrid
        ) < roadHalfWidth(z.float32 + 0.5'f - HalfGrid) + 1.0'f:
          continue
...
proc grassHabitat(layer: QuadLayer, x, z: int): tuple[elevation, slope: float32] =
  ...
  for dz in -2 .. 2:
    for dx in -2 .. 2:
      ...
      inc count
  if count > 0:
    result.elevation = elevation / (count.float32 * max(amplitude, 0.001'f))
    result.slope = sqrt(gradientX * gradientX + gradientZ * gradientZ) /
      count.float32

proc rebuildMaterials() =
  ## Assigns each grass tile one painting from broad terrain-aware regions.
...
var lastParams = (frequency, amplitude, octaves, gain, lacunarity, seed,
  fortEnabled, wallHeight, slopeLimit, treeDensity, grassCount,
    ...
    forestOctaves, forestGain, forestLacunarity, treeHeight, splatsPerTile,
    splatPlacement, textureSize, grassPatchSize)
...
window.onFrame = proc() =
  ...
  glUniform1f(roadEnabledLocation, if fortEnabled: 1.0 else: 0.0)
  ...
  ui:
    subWindow("Quad Terrain AI Generated", showPanel, vec2(10, 10), vec2(340, 680)):
      text "capture help":
