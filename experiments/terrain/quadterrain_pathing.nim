## Path-smoothing variant of the shadows quadterrain experiment. A* still
## finds the route tile by tile, one waypoint per square, so it can only
## step east/west/north/south and zigzags along every diagonal. The smoother
## then pulls the string tight: from each kept waypoint it keeps the
## farthest later waypoint that a straight line can reach, dropping every
## waypoint in between, and a line counts as clear only when walking it
## tile by tile crosses open edges of the same graph A* used — so a smoothed
## leg never cuts a cliff, a wall, or a blocked corner. The Path tab shows
## the raw tile path (dim) under the smoothed one (bright) and toggles both.
##
## Based on the shadow-mapped variant: a steerable
...
## Run from the repo root: nim r experiments/terrain/quadterrain_pathing.nim
...
var
  smoothPathVertexArray, smoothPathVertexBuffer: GLuint
  smoothPathMesh: seq[float32]
glGenVertexArrays(1, smoothPathVertexArray.addr)
glBindVertexArray(smoothPathVertexArray)
glGenBuffers(1, smoothPathVertexBuffer.addr)
glBindBuffer(GL_ARRAY_BUFFER, smoothPathVertexBuffer)
bindSolidPosition()
...
type
  ...
  PathNode = tuple[layer, x, z: int]  # a tile in a layer's local coordinates
...
var
  ...
  pathPoints: seq[Vec3]
  pathNodes: seq[PathNode]    # the A* route, one node per tile
  smoothNodes: seq[PathNode]  # the route pulled tight: kinks only
...
## Path smoothing
##
## A* walks the tile graph four ways, so its route zigzags along every
## diagonal. String pulling fixes that: a waypoint is kept only when the
## straight line from the previous kept waypoint to the one after it is
## blocked. "Clear" is decided by walking the line tile by tile (a grid
## traversal in world tile units) and asking the same edgeLink graph A* used
## whether each crossing is open, so a smoothed leg follows ramps onto the
## bridge, never steps over a cliff, and never cuts a blocked corner.

proc worldTile(node: PathNode): tuple[x, z: int] =
  ## A node's tile in world tile coordinates (layers have their own origins).
  (layers[node.layer].originX + node.x, layers[node.layer].originZ + node.z)

proc stepNode(node: PathNode, direction: int): tuple[open: bool, next: PathNode] =
  ## One edge crossing, possibly onto another layer (0 east, 1 south,
  ## 2 west, 3 north).
  let link = edgeLink(node.layer, node.x, node.z, direction)
  if link.open:
    (true, (link.layer, link.x, link.z))
  else:
    (false, node)

proc lineClear(a, b: PathNode, visited: var seq[PathNode]): bool =
  ## Whether a straight line between two tile centers stays on open edges.
  ## Amanatides-Woo grid traversal in world tile units; every axis step is
  ## one edge crossing. Where the line passes exactly through a tile corner
  ## both ways around the corner must be open and meet on the same tile,
  ## since a walker has width. `visited` collects the tiles the line crosses
  ## (for height lookups when drawing).
  visited.setLen(0)
  var node = a
  visited.add node
  let
    tileA = worldTile(a)
    tileB = worldTile(b)
    dx = float32(tileB.x - tileA.x)
    dz = float32(tileB.z - tileA.z)
  if tileA == tileB:
    return a == b
  let
    stepX = if dx > 0: 1 elif dx < 0: -1 else: 0
    stepZ = if dz > 0: 1 elif dz < 0: -1 else: 0
    # Line parameter advance per whole tile along each axis, and the
    # parameter at which the line first leaves the start tile on that axis
    # (centers sit at +0.5, so the first boundary is half a tile away).
    deltaX = if stepX != 0: 1.0'f32 / abs(dx) else: float32.high
    deltaZ = if stepZ != 0: 1.0'f32 / abs(dz) else: float32.high
  var
    tileX = tileA.x
    tileZ = tileA.z
    nextX = if stepX != 0: deltaX * 0.5 else: float32.high
    nextZ = if stepZ != 0: deltaZ * 0.5 else: float32.high
    guard = 0
  let
    dirX = if stepX > 0: 0 else: 2
    dirZ = if stepZ > 0: 1 else: 3
  while tileX != tileB.x or tileZ != tileB.z:
    inc guard
    if guard > 4 * GridTiles:
      return false
    if stepX != 0 and stepZ != 0 and abs(nextX - nextZ) < 1e-5:
      # Exactly through a corner: go around it both ways.
      let
        viaX = stepNode(node, dirX)
        viaXZ = if viaX.open: stepNode(viaX.next, dirZ) else: viaX
        viaZ = stepNode(node, dirZ)
        viaZX = if viaZ.open: stepNode(viaZ.next, dirX) else: viaZ
      if not (viaX.open and viaXZ.open and viaZ.open and viaZX.open):
        return false
      if viaXZ.next != viaZX.next:
        return false
      visited.add viaX.next
      visited.add viaZ.next
      node = viaXZ.next
      tileX += stepX
      tileZ += stepZ
      nextX += deltaX
      ...
      let crossing = stepNode(node, dirX)
      ...
      let crossing = stepNode(node, dirZ)
      if not crossing.open:
        return false
      node = crossing.next
      tileZ += stepZ
      nextZ += deltaZ
    visited.add node
  # The traversal must land on b itself, not a same-tile neighbour in
  # another layer (say the ground under the bridge deck).
  node == b

proc smoothPath(nodes: seq[PathNode]): seq[PathNode] =
  ## String pulling: from each kept waypoint, keep the farthest later
  ## waypoint a clear straight line reaches, dropping everything between.
  ## The result is the route's kinks only.
  if nodes.len <= 2:
    return nodes
  var visited: seq[PathNode]
  result.add nodes[0]
  var anchor = 0
  while anchor < nodes.len - 1:
    var reach = anchor + 1
    for candidate in countdown(nodes.len - 1, anchor + 2):
      if lineClear(nodes[anchor], nodes[candidate], visited):
        reach = candidate
        break
    result.add nodes[reach]
    anchor = reach

proc layerHeight(layerIndex: int, worldX, worldZ: float32): float32 =
  ## Bilinear top-surface height of one layer under a world xz position.
  let
    layer = layers[layerIndex]
    tx = worldX + HalfGrid - layer.originX.float32
    tz = worldZ + HalfGrid - layer.originZ.float32
    ix = clamp(int(floor(tx)), 0, layer.width - 1)
    iz = clamp(int(floor(tz)), 0, layer.depth - 1)
    h = layer.tiles[iz * layer.width + ix].tops.unpack
    fx = clamp(tx - ix.float32, 0, 1)
    fz = clamp(tz - iz.float32, 0, 1)
  (h[0] * (1 - fx) + h[1] * fx) * (1 - fz) + (h[2] * (1 - fx) + h[3] * fx) * fz

proc smoothPathPoints(): seq[Vec3] =
  ## The smoothed route as a dense polyline hugging the ground: each leg is
  ## sampled along its straight line and each sample takes the height of
  ## whichever layer the line is crossing there, so a leg up the bridge
  ## ramp climbs with the deck instead of cutting through it.
  const SampleStep = 0.2'f32
  var visited: seq[PathNode]
  for i in 0 ..< max(0, smoothNodes.len - 1):
    let
      a = smoothNodes[i]
      b = smoothNodes[i + 1]
      pa = tileCenter(a.layer, a.x, a.z)
      pb = tileCenter(b.layer, b.x, b.z)
    discard lineClear(a, b, visited)
    let
      span = vec2(pb.x - pa.x, pb.z - pa.z)
      samples = max(1, int(ceil(span.length / SampleStep)))
    for s in 0 .. samples:
      if i > 0 and s == 0:
        continue  # the previous leg ended here
      let
        t = s.float32 / samples.float32
        x = pa.x + span.x * t
        z = pa.z + span.y * t
        tileX = int(floor(x + HalfGrid))
        tileZ = int(floor(z + HalfGrid))
      var layerIndex = a.layer
      for node in visited:
        if worldTile(node) == (tileX, tileZ):
          layerIndex = node.layer
          break
      result.add vec3(x, layerHeight(layerIndex, x, z), z)

proc ribbon(
    points: seq[Vec3], halfWidth, lift: float32, mesh: var seq[float32],
    buffer: GLuint
) =
  ## A flat ribbon of quads along a polyline, lifted a little so it doesn't
  ## z-fight the terrain.
  mesh.setLen(0)
  for i in 0 ..< max(0, points.len - 1):
    let
      a = points[i] + vec3(0, lift, 0)
      b = points[i + 1] + vec3(0, lift, 0)
      flat = vec3(b.x - a.x, 0, b.z - a.z)
    if flat.length < 0.0001:
      continue
    let side = normalize(vec3(-flat.z, 0, flat.x)) * halfWidth
...
proc rebuildPathMesh() =
  ## The raw tile path is a thin ribbon through the tile centers; the
  ## smoothed path a wider one, lifted a touch higher so it draws on top.
  ribbon(pathPoints, 0.08, 0.22, pathMesh, pathVertexBuffer)
  ribbon(smoothPathPoints(), 0.14, 0.3, smoothPathMesh, smoothPathVertexBuffer)

proc recomputePath() =
  pathNodes = findPath()
  pathPoints.setLen(0)
  for node in pathNodes:
    pathPoints.add tileCenter(node.layer, node.x, node.z)
  smoothNodes =
    if smoothPathEnabled: smoothPath(pathNodes)
    else: pathNodes
  if hasStart and hasFinish:
    if pathNodes.len > 0:
      pathStatus = &"{pathNodes.len} tiles, {smoothNodes.len} waypoints."
    else:
...
var lastSmoothPath = smoothPathEnabled
...
when defined(takeScreenshot):
  ...
  if existsEnv("PATH_START") and existsEnv("PATH_FINISH"):
    ...
    recomputePath()
    recomputeVisibility()
  if existsEnv("SMOOTH_PATH"): smoothPathEnabled = getEnv("SMOOTH_PATH") != "0"
  if existsEnv("RAW_PATH"): showRawPath = getEnv("RAW_PATH") != "0"
...
window.onFrame = proc() =
  ...
  if losEnabled != lastLosEnabled:
    ...
    recomputeVisibility()
  if smoothPathEnabled != lastSmoothPath:
    lastSmoothPath = smoothPathEnabled
    recomputePath()
  if tieToTime:
    applyTimeOfDay()
  ...
  if showRawPath and pathMesh.len > 0:
    # The tile-by-tile route underneath, dimmed.
    glUniform3f(solidColorLocation, 0.55, 0.42, 0.12)
    glBindVertexArray(pathVertexArray)
    glDrawArrays(GL_TRIANGLES, 0, (pathMesh.len div 3).GLsizei)
  if smoothPathMesh.len > 0:
    glUniform3f(solidColorLocation, 1.0, 0.85, 0.2)
    glBindVertexArray(smoothPathVertexArray)
    glDrawArrays(GL_TRIANGLES, 0, (smoothPathMesh.len div 3).GLsizei)
  if hasStart:
    mvp = viewProjection *
  ...
  ui:
    subWindow("Quad Terrain Pathing", showPanel, vec2(10, 10), vec2(340, 680)):
      group "tab row 1":
        box 310, 32
        layout LeftToRight
        itemSpacing 10
      ...
      of PathTab:
        ...
        text "smoothing title":
          characters "Smoothing"
        checkBox "Pull the path straight", smoothPathEnabled
        text "smoothing hint":
          characters "Keeps only the kinks A* needs"
        checkBox "Show tile path underneath", showRawPath
        text "help markers":
  ...
  when defined(takeScreenshot):
    ...
    if screenshotFrame == 30:
      ...
      image.flipVertical()
      image.writeFile("quadterrain_pathing_shot.png")
      quit(0)
