## Textured variant of the quadterrain experiment: instead of flat per-kind
...
## Run from the repo root: nim r experiments/terrain/quadterrain_blended.nim
...
proc treeFrag(
  ...
  let light = clamp(
    abs(dot(normalize(fragmentNormal), normalize(vec3(0.45, 0.8, 0.4)))) * 0.5 + 0.5,
    0.0, 1.0)
  var color = texel.xyz * (0.5 + 0.65 * light)
  let
    visUv = vec2(
...
type PanelTab = enum
  TerrainTab, TexturesTab, TreesTab, LayersTab, PathTab
...
window.onFrame = proc() =
  ...
  ui:
    subWindow("Quad Terrain Blended", showPanel, vec2(10, 10), vec2(310, 680)):
      group "tab row 1":
        box 280, 32
        layout LeftToRight
        itemSpacing 10
  ...
  when defined(takeScreenshot):
    ...
    if screenshotFrame == 30:
      ...
      image.flipVertical()
      image.writeFile("quadterrain_blended_shot.png")
      quit(0)
