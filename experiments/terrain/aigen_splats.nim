...
type
  AigenError* = object of CatchableError
  Splat* = object
    position*: Vec2
...
proc generateSplats*(
  ...
  if width <= 0 or depth <= 0 or width > int.high div depth or
    tiles.len != width * depth or count < 0:
      raise newException(AigenError, "Invalid splat map dimensions or count.")
  for tile in tiles:
    if tile.material < 0 or tile.material >= materialCount or tile.variants < 0:
      raise newException(AigenError, "Invalid splat material.")
    let
    ...
    if variants > materialCount - first:
      raise newException(AigenError, "Splat variants exceed the material array.")
  if count > 1_000_000 div tiles.len:
    raise newException(AigenError, "Splat placement budget exceeds one million.")
  if not (chance >= 0 and chance <= 1) or not (size > 0 and size <= 12):
    raise newException(AigenError, "Invalid splat placement chance or scale.")
  var
    rng = initRand(seed)
