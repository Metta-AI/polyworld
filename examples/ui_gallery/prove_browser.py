...
def wait(expression):
    browser("wait", "--fn", "(() => { const state = window.__polyworldUiGallery?.snapshot(); "
...
try:
    ...
    click(150, 600)
    wait("state.difficulty === 'Hard'")
    browser("screenshot", "tmp/ui-gallery/settings.png")
    click(150, 575)
    click(150, 650)
    wait("state.theme === 'Parchment'")
