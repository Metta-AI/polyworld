...
const
  GotaEnvVersion = 1
  EnvMagic = 0x474F5441454E5631'u64 ## "GOTAENV1"
  NetMagic = 0x474F54414E455431'u64 ## "GOTANET1"
  StatCount = 24
  OrderSize = 16
...
type
  ...
  Env = ref object
    magic: uint64
      ## EnvMagic while the handle is live (toEnv rejects anything else).
    config: GotaConfig
    ...
    defers: array[10, string]
      ## The learner seat's defer script (empty = off).
    root: string
    goals: array[10, array[GoalSize, float32]]
    status: array[10, SeatStatus]
    notes: array[10, string]
      ## The last rejected configuration of each seat (kept across resets,
      ## cleared when the seat is configured successfully).
    game: Game
    seeds: int64
...
proc resetEnv(env: Env, seed: int64): int =
  ...
  result = 0
  for i in 0 ..< 10:
    env.status[i] = SeatStatus(code: 1, message: env.notes[i])
    env.prevScore[i] = 0
    env.pushDepth[i] = 0
...
type NetHandle = ref object
  magic: uint64
  actor: Actor

proc toEnv(handle: pointer): Env =
  ## The live env behind a handle; nil for nil, destroyed or foreign handles.
  if handle == nil:
    return nil
  result = cast[Env](handle)
  if result.magic != EnvMagic:
    lastError = "not a live gota env handle"
    return nil
  if result.game != nil:
...
proc gota_mask_size(): cint {.exportc, dynlib, cdecl.} = MaskSize
...
proc gota_set_seat_defer_script(handle: pointer, seat: cint, path: cstring): cint {.exportc, dynlib, cdecl.} =
  ## Defer script: verb 0 defers to this script on a learner seat
...
proc gota_action_mask(handle: pointer, seat: cint, output: ptr UncheckedArray[uint8]): cint {.exportc, dynlib, cdecl.} =
  ...
  if s != nil and s.frameTick == game.world.tick:
    mask = if s.mode == NeuralHosted and s.maskMode != NoMask: s.mask
...
proc gota_set_seat_package(handle: pointer, seat: cint, zip: pointer, length: int64): cint {.exportc, dynlib, cdecl.} =
  ...
  try:
    let package = parseGotaPackage(bytes)
    ...
    env.notes[seat] = env.status[seat].message
    return 2
  env.notes[seat] = ""
...
proc gota_net_load(data: pointer, length: int64, error: ptr char, capacity: int32): pointer {.exportc, dynlib, cdecl.} =
  ...
  try:
    let handle = NetHandle(magic: NetMagic, actor: loadActor(bytes, GotaContract))
    GC_ref(handle)
    cast[pointer](handle)
  except CatchableError as e:
    copyText(e.msg, error, capacity)
...
proc toNet(net: pointer): Actor =
  ## The actor behind a live net handle; nil otherwise.
  if net == nil:
    return nil
  let handle = cast[NetHandle](net)
  if handle.magic != NetMagic:
    lastError = "not a live gota net handle"
    return nil
  handle.actor
...
proc gota_net_info(net: pointer, eight: ptr UncheckedArray[int64]): cint {.exportc, dynlib, cdecl.} =
  let actor = toNet(net)
  if actor == nil or eight == nil: return -1
  eight[0] = 1
...
proc gota_net_infer(net: pointer, observation, state, logits: ptr UncheckedArray[float32]): cint {.exportc, dynlib, cdecl.} =
  let actor = toNet(net)
  if actor == nil or observation == nil or state == nil or logits == nil: return -1
  try:
