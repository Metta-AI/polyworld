...
proc handlePlayerKeys*(state: var HumanInputState, keys: HumanKeys,
  ...
  for slot in 0 .. 3:
    if (abilityKey(slot) in keys.pressed):
      if KeyLeftShift in keys.held or KeyRightShift in keys.held:
        cancelPlayerAim()
        queueLevelAbility(heroId, slot.int32)
      else:
  ...
  if (KeyF in keys.pressed): activatePlayerItem(world, heroId, 0)
  if (KeyG in keys.pressed): activatePlayerItem(world, heroId, 1)
