...
import
  ...
  polyworld/neural,
  bassy, fixxy,
  polyworld/[mailboxes, metrics, bodies, cli, controllers, neural_host,
...
proc installPackageSeat*(game: Game, i: int, bytes: string) =
  ...
  try:
    package = parseGotaPackage(bytes)
  except CatchableError:
    let message = "neural package rejected: " & getCurrentExceptionMsg()
    when defined(coworld):
      failPlayer(i, message, "neural package rejected for player slot " & $i)
    raise newException(BasicError, message)
  let
    options = package.gotaOptions
    limits = if options.deferScript: deferVmLimits() else: neuralVmLimits()
  ...
  let seat = newNeuralSeat(NeuralHosted, package.decisionPeriod,
    game.config.maxTicks)
  seat.actor = package.actor
  seat.goal = options.goals[game.world.heroes[i].team.ord]
  seat.sampling = package.decoder.sampling
  seat.temperature = package.decoder.temperatureOf
  seat.telemetry = true
  seat.deferEnabled = options.deferScript
  seat.maskMode = options.maskMode
...
proc runHeroVm(game: Game, index: int, vm: HeroVm, primary: bool) =
  ...
  if vm == nil or vm.failed:
    return
  try:
    if primary and vm.neural != nil and NeuralSeat(vm.neural).deferEnabled:
