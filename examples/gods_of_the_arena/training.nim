...
proc advance*(lane: TrainingLane, action: int32 = 0) =
  ...
  while true:
    if not lane.inTick:
      if game.world.gameOver or game.world.battleTick() >= lane.maxTicks:
        lane.snapshot(true)
        return
...
proc installTrainingPolicy(lane: TrainingLane, seat: int, source: string) =
  let game = lane.game
  var host = initHeroHost(game.world.heroes[seat].id)
...
proc newLane(batch: TrainingBatch, seed: int): TrainingLane =
  ...
  let game = newGame(
    gameMap, batch.config.spawnIntervalTicks, 10, false, ReplayData(), true
  )
