...
##
## GotA is a client of the shared neural tier (src/polyworld/neural_*.nim,
## docs/neural-policies.md); this module is the GotA half. It owns the
## fixed-size ego-centric observation, the five-head action decoder, the
## demonstration encoder and the GotA package options (goal, defer_script,
## mask_empty_targets), so the hosted neural seat,
...
import
  ...
  crunchy, fixxy,
  polyworld/[bodies, metrics, neural_host, pathing],
...
proc argmaxHeads*(logits: openArray[float32]): Heads =
  ...
  argmaxHeads(logits, HeadSizes, result)

# Package options (the GotA extension keys of a gota-neural-basic/1 manifest).

const
  PackageSchema* = "gota-neural-basic/1"
  MaxDecisionPeriod* = 24
  GotaManifestKeys* = ["goal"]
  GotaDecoderKeys* = ["defer_script", "mask_empty_targets", "mask_mode"]

type
  MaskMode* = enum
    NoMask ## decoder.mask_empty_targets absent or false
    ConditionalMask ## mask_mode "conditional" (the default)
    StaticMask ## mask_mode "static"
  GotaPackageOptions* = ref object of RootObj
    goals*: array[2, array[GoalSize, float32]] ## red, blue
    deferScript*: bool
      ## decoder.defer_script: verb 0 defers to policy.bas, which then acts
      ## for that decision window.
    maskMode*: MaskMode
      ## decoder.mask_empty_targets + mask_mode: action validity mask
...
proc parseGotaOptions*(manifest: JsonNode): RootRef {.nimcall.} =
  ## The GotA manifest keys: goal {red, blue} and the decoder options.
  let options = GotaPackageOptions(goals: [defaultGoal(), defaultGoal()])
  if manifest.hasKey("goal"):
    let goal = manifest["goal"]
    goal.requireManifestKeys(["red", "blue"], "goal")
    if not goal.hasKey("red") or not goal.hasKey("blue"):
      raise newException(ValueError, "goal needs red and blue")
    options.goals[0] = goalVector(goal["red"], "goal.red")
    options.goals[1] = goalVector(goal["blue"], "goal.blue")
  if manifest.hasKey("decoder"):
    let decoder = manifest["decoder"]
    let masked = decoder.requireBool("mask_empty_targets", "decoder")
    if masked:
      options.maskMode = ConditionalMask
    if decoder.hasKey("mask_mode"):
      if not masked:
        raise newException(ValueError, "decoder.mask_mode needs mask_empty_targets true")
      case (if decoder["mask_mode"].kind == JString: decoder["mask_mode"].getStr else: "")
      of "conditional": discard
      of "static": options.maskMode = StaticMask
      else:
        raise newException(ValueError, "decoder.mask_mode must be conditional or static")
    options.deferScript = decoder.requireBool("defer_script", "decoder")
  options
