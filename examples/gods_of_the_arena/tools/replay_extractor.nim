...
The default replay is tmp/replays/gota.replay, generated by tools/gen_replays.nim."""
        quit(0)
    of "--checkpoint-seconds", "--evidence", "--events":
        ...
        inc i
        if argument == "--events":
            let bounds = paramStr(i).split(':')
            if bounds.len > 2:
                quit("--events requires START or START:END", 1)
            eventStart = natural(bounds[0], "--events")
            eventEnd = eventStart
            if bounds.len == 2:
                eventEnd = natural(bounds[1], "--events")
            if eventEnd < eventStart:
                quit("--events END must not precede START", 1)
            if eventEnd - eventStart >= MaximumEventWindow:
...
echo "REPLAY"
echo &"  path={replayPath} version={ReplayGameVersion} " &
...
for tick in 1 .. replay.hashes.len:
    ...
    for event in game.world.events:
        ...
        of Damage:
            if actor != nil:
                actor[].damageDone += event.amount
            if target != nil:
                target[].damageTaken += event.amount
        of Healing:
            if actor != nil:
                actor[].healingDone += event.amount
        of Death:
            if actor != nil:
            ...
            if target != nil:
                inc target[].deaths
            if event.target.kind in {HeroObjectKind, FortObjectKind,
                    TowerObjectKind, BarracksObjectKind}:
                summary.remember(
        ...
        of GoldGained:
            if target != nil:
                target[].goldGained += event.amount
        of GoldSpent:
            if target != nil:
                target[].goldSpent += abs(event.amount)
        of XpGained:
            if target != nil:
                target[].xpGained += event.amount
        of ActionRejected:
            if actor != nil:
                inc actor[].rejected
        ...
        of MatchEnded:
            ...
            summary.remember(summary.matchEnd, evidenceLimit)
        else:
            discard
        if event.tick >= eventStart and event.tick <= eventEnd:
            if summary.rawEvents.len < MaximumRawEvents:
                summary.rawEvents.add $event
            else:
                inc summary.omittedRawEvents
    if checkpointSeconds > 0 and
            ((tick mod tickRate == 0 and
                (tick div tickRate) mod checkpointSeconds == 0) or
    ...
    for kind, count in value.actions:
        if count > 0:
            let name =
                ...
                else: "unknown-" & $kind.uint8
            actions.add name & "=" & $count
...
for line in summary.lastEvidence:
    echo "  " & line
if eventStart >= 0:
    echo &"RAW_EVENTS ticks={eventStart}:{eventEnd}"
