...
from native_env import Env, Lib, NativeEnvError, random_actions, HEAD_SIZES
...
def test_goal(lib):
    ...
    try:
        env.set_goal(6, bad); code = 0
    except NativeEnvError as e:
        code = e.code
    check("goal w_reserved != 0 rejected", code == -3)
    env.reset(11)
    obs, _, _ = env.observe()
...
def test_validation(lib):
    ...
    cases = {
        ...
        "float integer inputs": lambda f: f.__setitem__("manifest.json", json.dumps(
            dict(json.loads(f["manifest.json"]), model=dict(json.loads(f["manifest.json"])["model"], inputs=1407.0))).encode()),
        "temperature true": edit_manifest("decoder", {"mode": "sample", "temperature": True}),
        "non-finite goal": lambda f: f.__setitem__("manifest.json", f["manifest.json"].replace(
            b'"decision_period": 4', b'"decision_period": 4, "goal": {"red": [1e999' + b', 0' * 15 +
            b'], "blue": [0' + b', 0' * 15 + b']}')),
        "manifest not json": lambda f: f.__setitem__("manifest.json", b"{not json"),
    }
    env = Env(lib, learner_seats=[])
    ...
    for name, fn in list(cases.items()) + [("truncated package", None)]:
        bad = corrupt(good, fn) if fn else good[:len(good) // 2]
        try:
