 ...
 * (build command: the header comment of native_env.nim). One handle owns one
 * ten-seat match.
...
/* Opaque handles. A destroyed or foreign handle is rejected (-1 / NULL)
 * when the library can tell; never pass freed handles. */
typedef struct GotaEnv GotaEnv;
typedef struct GotaNet GotaNet;
...
int gota_mask_size(void);                   /* GOTA_MASK_SIZE */
...
GotaEnv *gota_create(const char *config_json, char *error, int32_t capacity);
void gota_destroy(GotaEnv *handle);
...
int gota_reset(GotaEnv *handle, int64_t seed);

/* Learner seat mask (bit s = seat s). Takes effect at the next gota_reset. */
int gota_set_learner_seats(GotaEnv *handle, uint32_t mask);
uint32_t gota_learner_seats(GotaEnv *handle);
int gota_seat_team(GotaEnv *handle, int seat);        /* 0 red, 1 blue */
int gota_seat_class(GotaEnv *handle, int seat);       /* drafted HeroClass, -1 */
int gota_decision_period(GotaEnv *handle);
...
int gota_observe_seats(GotaEnv *handle, uint32_t seats, float *obs, float *resets,
                       float *acting);
 ...
 * -2 already over, -1 bad args, -3 internal error (gota_last_error): the
 * episode is then in an undefined state and the caller must gota_reset
 * before stepping again. */
int gota_step(GotaEnv *handle, const int32_t *actions, float *rewards,
              float *terminals);

/* float[8] = {over, winner (0 red, 1 blue, -1 none), draw, time_limit,
 * battle_ticks, red_god_hp, blue_god_hp, total_ticks}. */
int gota_results(GotaEnv *handle, float *eight);
uint64_t gota_state_hash(GotaEnv *handle);  /* the replay hash of the current tick */
int32_t gota_battle_tick(GotaEnv *handle);
...
int gota_seat_stats(GotaEnv *handle, int seat, int64_t *out);
...
int gota_set_seat_script(GotaEnv *handle, int seat, const char *source, int32_t length);
int gota_set_policy_script(GotaEnv *handle, const char *source, int32_t length);
...
int gota_seat_script_status(GotaEnv *handle, int seat, char *message, int32_t capacity);
...
int gota_seat_orders(GotaEnv *handle, int seat, int32_t *out);

/* Mapping ceiling (tools/mapping_ceiling.py). With enabled = 1 a SCRIPTED
...
int gota_set_seat_override(GotaEnv *handle, int seat, int32_t enabled);
...
int gota_set_seat_shadow(GotaEnv *handle, int seat, const char *source, int32_t length);

/* Defer script: verb 0 becomes DEFER on this LEARNER seat.
...
int gota_set_seat_defer_script(GotaEnv *handle, int seat, const char *script_path);
...
int gota_seat_defer_stats(GotaEnv *handle, int seat, int64_t *out);
...
int gota_action_mask(GotaEnv *handle, int seat, uint8_t *out);
...
int gota_set_seat_package(GotaEnv *handle, int seat, const void *zip, int64_t length);

/* Neural actor (neural_actor.nim), bit-identical to the hosted seat, so a
 * trainer can check its FP32 actor against the deployed one bit for bit.
 * model.bin format GOTANET1 (docs/neural-policies.md). gota_net_load refuses
 * models over 4,000,000 operations per inference and models whose header
 * contract hashes, input size or head sizes are not GotA contract v1; NULL
 ...
 * (reason via gota_last_error). */
GotaNet *gota_net_load(const void *data, int64_t length, char *error, int32_t capacity);
void gota_net_destroy(GotaNet *net);
int gota_net_info(GotaNet *net, int64_t *eight);
int gota_net_infer(GotaNet *net, const float *observation, float *state, float *logits);
